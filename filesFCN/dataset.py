...
import numpy as np
import abtem.waves as waves
from abtem.noise import poisson_noise
from ase import io
from filesFCN.labels import create_label
from filesFCN import mods
...
class DataEntry(object):
    ...
    def load(self):
        ...
        if self.wave_file is not None:
            self._wave = waves.Waves.read(self.wave_file)
    ...
    def create_image(self, ctf, sampling, dose, noise, concatenate=False):
        measurement = self._wave.apply_ctf(ctf).intensity().mean(0)
        # image = poisson_noise(measurement,noise).mean(0)
    ...
    def create_label(self,sampling,shape=None,width=12,num_classes=None):
        ...
        self._label=create_label(self._sites[:,:2],shape,width=width,classes=self._classes,null_class=True,num_classes=num_classes)
        self._label=self._label.reshape((1,)+self._label.shape)
    ...
    def view(self,axes=None,show_positions=True):
        ...
        axes[0].imshow(self._image.T,cmap='gray')
        axes[1].imshow(self._label[0,:,:].T,cmap='gray')
